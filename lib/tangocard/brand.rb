class Tangocard::Brand
  ...
  #   none
  def self.all
    response = Tangocard::Raas.rewards_index
    if response.success?
      return response.parsed_response['brands'].map{|p| Tangocard::Brand.new(p)}
    else
      raise Tangocard::BrandGetRewardsFailedException, "#{response.error_message}"
    end
  end
